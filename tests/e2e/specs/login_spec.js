...
describe('Login', () => {
  ...
  before(() => {
    cy.wrap(
      mailslurp.getAllEmails().then((emails) =>
        emails.content.forEach((e) => {
  ...
  xit('resets password', () => {
    ...
    cy.get('[data-test=forgot-password-email]').type(
      newCustomer.email
    );
    cy.get(
    ...
    cy.wrap(
      mailslurp
        .waitForEmailCount(
          1,
          msEmail.substring(0, msEmail.lastIndexOf('@'))
        )
        .then((response) =>
          mailslurp
            .getEmail(response[0].id)
            .then(
              (fullEmail) =>
                fullEmail.body.match(/a href="([^"]*)/)[1]
            )
        )
    ).then((link) => {
      cy.visit(link);
      cy.get('[data-test=reset-new-password]').type(
        newPassword
      );
      cy.get('[data-test=reset-confirm-password]').type(
        newPassword
      );
